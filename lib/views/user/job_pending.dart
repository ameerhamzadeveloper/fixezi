...
import 'package:fixezi/views/user/view_quoted_job_details.dart';
...
import '../splash.dart';
class JobPendingsUser extends StatefulWidget {
  @override
  _JobPendingsUserState createState() => _JobPendingsUserState();
}

class _JobPendingsUserState extends State<JobPendingsUser> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        onRefresh: ()async{
          return pro.fetchJobs(authPro.userId,'1',1);
        },
        child: Padding(
            padding: const EdgeInsets.all(8.0),
            child: pro.userPendingJobs == null ? Center(
              child: CircularProgressIndicator(),
            ): pro.userPendingJobs!.isEmpty ? RefreshIndicator(onRefresh: ()async{
               return pro.fetchJobs(authPro.userId,'1',1);
            ...
            RefreshIndicator(
              ...
              child: ListView.builder(
                itemCount: pro.userPendingJobs!.length,
                itemBuilder: (c,i){
                  var d = pro.userPendingJobs![i];
                  return InkWell(
                    onTap: (){
                      pro.setCurrentJobPendingId(d.jobId!);
                      if(d.statusText == "QUOTED"){
                        nextScreen(context, JobQuotedDetails(index: i,));
                      }
                          ...
                          children: [
                            Row(
                              ...
                              children: [
                                ...
                                Text(d.statusText!,style: GoogleFonts.poppins(color:d.statusText == 'QUOTED'? Colors.pinkAccent: Colors.black,fontSize: 18,fontWeight: FontWeight.bold),),
                              ],
                            ),
                            SizedBox(height: 10,),
